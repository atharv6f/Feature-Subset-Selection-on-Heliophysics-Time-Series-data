...
def main():
  ...
  for KERNEL in ['linear','rbf']:
    for RANKING_FILE in ['csfs_rank.csv','clever_rank.csv','corona_ranks.csv','fcbf_rank.csv','pie_rank.csv']:
      RANKING_ALGO = RANKING_FILE.split("_")[0]

      feature_set = pd.read_csv(os.path.join(RESULTS_PATH,RANKING_FILE)).reset_index()
      clf = TimeSeriesSVC(kernel = KERNEL)

      print(f"{KERNEL=}, {RANKING_FILE=}, univariate")
      univariate_result = univariate(sampled_file_path = sampled_file_path, feature_set=feature_set, 
                      clf=clf, X_train=X_train, y_train=y_train)
      save_table(RESULTS_PATH, univariate_result, prepare_path("univariate",RANKING_ALGO,KERNEL))

      print(f"{KERNEL=}, {RANKING_FILE=}, multivariate")
      multivariate_result = multivariate(sampled_file_path = sampled_file_path, feature_set=feature_set, 
                      clf=clf, X_train=X_train, y_train=y_train)
      save_table(RESULTS_PATH, multivariate_result, prepare_path("multivariate",RANKING_ALGO,KERNEL))
